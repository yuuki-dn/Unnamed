...
class GuildData:
    ...
    async def __fetch_reaction_role_message__(self, message_id: int, guild_id: int) -> ReactionRoleMessageEntity | None:
        try:
            result: list = await self.database.execute_query("SELECT emoji, role_id FROM reaction_role_messages WHERE message_id = %s AND guild_id = %s", (message_id, guild_id))
    ...
    async def update_guild(self, entity: GuildEntity) -> None:
        try:
            ...
            if previous is None:
                await self.database.execute_update("INSERT INTO guilds (guild_id, wordchain_channel_id) VALUES (%s, %s)", (entity.guild_id, entity.wordchain_channel_id))
            else:
                await self.database.execute_update("UPDATE guilds SET wordchain_channel_id = %s WHERE guild_id = %s", (entity.wordchain_channel_id, entity.guild_id))
    ...
    async def update_reaction_role_message(self, entity: ReactionRoleMessageEntity) -> None:
        try:
            ...
            for key in new_key.difference(previous_key):
                # insert new
                await self.database.execute_update("INSERT INTO reaction_role_messages (message_id, guild_id, emoji, role_id) VALUES (%s, %s, %s, %s)", (entity.message_id, entity.guild_id, key, entity.map[key]))
            
            for key in new_key.intersection(previous_key):
                # update exists
                await self.database.execute_update("UPDATE reaction_role_messages SET role_id = %s WHERE message_id = %s AND guild_id = %s AND emoji = %s", (entity.map[key], entity.message_id, entity.guild_id, key))
                
            for key in previous_key.difference(new_key):
                # delete
                await self.database.execute_update("DELETE FROM reaction_role_messages WHERE message_id = %s AND guild_id = %s AND emoji = %s ", (entity.message_id, entity.guild_id, key))
    ...
    async def delete_reaction_role_message(self, message_id: int, guild_id: int) -> None:
        try:
            await self.database.execute_update("DELETE FROM reaction_role_messages WHERE message_id = %s AND guild_id = %s", (message_id, guild_id))
            self.guild_cache.delete(guild_id)
