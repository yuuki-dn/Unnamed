...
class GuildData:
    ...
    async def update_guild(self, entity: GuildEntity) -> None:
        try:
            ...
            if previous is None:
                await self.database.execute_update("INSERT INTO `guilds` (`guild_id`, `wordchain_channel_id`) VALUE (%s, %s)", (entity.guild_id, entity.wordchain_channel_id))
            else:
    ...
    async def update_reaction_role_message(self, entity: ReactionRoleMessageEntity) -> None:
        try:
            ...
            for key in new_key.difference(previous_key):
                # insert new
                await self.database.execute_update("INSERT INTO `reaction_role_messages` (`message_id`, `guild_id`, `emoji`, `role_id`) VALUE (%s, %s, %s, %s)", (entity.message_id, entity.guild_id, key, entity.map[key]))
